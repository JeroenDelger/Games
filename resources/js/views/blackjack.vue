...
// TODO: gebruik Engelse naamgeving
...
function Playingcards() {
    for (let i = 0; i < Suits.length; i++) {
        // TODO: gebruik j als iterator (alfabetisch oplopend)
        for (let x = 0; x < Cards.length; x++) {
            // TODO: gebruik camelCase
            let card = {
                Card: Cards[x],
                Suit: Suits[i],
                Points: Points[x],
...
function GetCard() {
    // TODO: push functie is leesbaarder
    PlayerCards.value[PlayerCards.value.length] = Deck.value.splice(
        Math.floor(Math.random() * Deck.value.length),
        1
    )[0];
    PuntenBerekeningPlayer();
}

// TODO: maak hier een computed property van, zodat dit automatisch berekend wordt
// TODO: berekening score player en dealer kan via 1 functie (is veel overlap tussen
// beide functies)
function PuntenBerekeningPlayer() {
    ...
    if (PlayerAce.value > 0 && PlayerPoints.value + 10 + PlayerAce.value < 22) {
        ...
        PlayerAce.value > 0 &&
        PlayerPoints.value + 10 + PlayerAce.value > 21
    ) {
...
function DealerCard() {
    DealerCards.value[DealerCards.value.length] = Deck.value.splice(
        Math.floor(Math.random() * Deck.value.length),
        1
    )[0];
