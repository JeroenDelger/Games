    ...
    <div>
        <table>
            ...
            <tr>
                ...
                <td
                    ...
                    :style="{
                        ...
                        background: DobbelsteenBackground[i - 1],
                    }"
                    @click="SteenKeuze(i - 1)"
                >
                    {{ Dobbelstenen[i - 1] }}
                </td>
                <td>
                    <button @click="WerpStenen(Dobbelstenen)">
                        Gooi Stenen
                    </button>
                </td>
            </tr>
            ...
            <tr v-for="i in 6">
                ...
                <td
                    ...
                    :style="{
                        ...
                        height: '50px',
                        border: '1px solid black',
                        background: BackgroundColorBoven[n - 1][i - 1],
                    }"
                    :id="n"
                >
                    <!-- <div v-if="n === Game"> -->
                    ...
                    <div
                        v-if="
                            VastzettenBoven[n - 1][i - 1] == false && n === Game
                        "
                        @click="
                            ScoreVastzettenBoven(i, $event.target.innerHTML)
                        "
                    >
            ...
            <tr v-for="i in 7" :id="i">
                ...
                <!-- TODO: als je de onderstaande for-loop verijwderd, en de 7 scores
                 hard-code, dan spaar je 7 v-if statements uit en kost het je niet meer code-->
                <td
                    v-for="n in 6"
                    :style="{
                        width: '100px',
                        height: '50px',
                        border: '1px solid black',
                        background: BackgroundColorBeneden[n - 1][i - 1],
                    }"
                    :id="n"
                >
                    ...
                    <div
                        v-if="
                            n === Game &&
                            VastzettenOnder[n - 1][i - 1] == false &&
                            Worp > 0
                        "
                        @click="
                            ScoreVastzetenBeneden(i, $event.target.innerHTML)
                        "
                    >
                        <div v-if="i == 1">
                            {{ TreeOfAKind() }}
...
const BovensteRijScoreNamen = [
    "Score boven",
    "Bonus punten",
    "Totaalscore boven",
];
const NamenOndersteRij = [
    "Tree of a kind",
    "Carre",
    "Full House",
    "Kleine straat",
...
function ScoreVastzettenBoven(i, score) {
    if (Worp.value > 0) {
        ...
        Worp.value = 0;
        // TODO: resetten van variablen is repeterende code in deze en onderstaande functie, kan
        // efficienter door dit in een aparte functie te zetten
        Dobbelstenen.value = [0, 0, 0, 0, 0];
        DobbelsteenBackground.value = [
            "white",
            "white",
            "white",
            "white",
...
function TotaalScoreBerekening() {
    ...
    if (ScoreCheck1 == true && ScoreCheck2 == true) {
        ...
        return (
            ScoreOnder.value[Game.value - 1].reduce((a, b) => a + b, 0) +
            ScoreBoven.value[Game.value - 1].reduce((a, b) => a + b, 0)
        );
    }
}
...
function Bonus() {
    let ScoreCheck = VastzettenBoven.value[Game.value - 1].every(Boolean);
    if (
        ScoreBoven.value[Game.value - 1].reduce((a, b) => a + b, 0) > 62 &&
        ScoreCheck == true
    ) {
        return 35;
...
// TODO: repeterende code in ThreeOfAKind en FourOfAKind, kan korter / efficienter
...
function FullHouse() {
    ...
    FullHouseCheck.sort();
    if (
        (FullHouseCheck[0] == FullHouseCheck[1] &&
            FullHouseCheck[2] == FullHouseCheck[4]) ||
        (FullHouseCheck[0] == FullHouseCheck[2] &&
            FullHouseCheck[3] == FullHouseCheck[4]) ||
...
function KleineStraatBerekening() {
    ...
    let KleineStraatLaag = KleineStraat[0].every((val) =>
        StraatCheck.includes(val)
    );
    let KleineStraatMidden = KleineStraat[1].every((val) =>
        StraatCheck.includes(val)
    );
    let KleineStraatHoog = KleineStraat[2].every((val) =>
        StraatCheck.includes(val)
    );
...
function GroteStraatBerekening() {
    var StraatCheck = JSON.parse(JSON.stringify(Dobbelstenen.value));
    let GroteStraatLaag = GroteStraat[0].every((val) =>
        StraatCheck.includes(val)
    );
    let GroteStraatHoog = GroteStraat[1].every((val) =>
        StraatCheck.includes(val)
    );
...
// TODO: computed van alle scores maken zodat deze automatisch updaten
function Chance() {
